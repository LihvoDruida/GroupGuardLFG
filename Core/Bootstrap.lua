...
addon.L10N = {
  enUS = {
    ...
    APPLICANT_TOOLTIP_SUPPLEMENT = "supplement",
    ...
    APPLICANT_DUMP_MEMBER = "  member %s | name=%s role=%s specID=%s ilvl=%s pvp=%s score=%s lvl=%s leaver=%s listingRun=%s context=%s source=%s",
  ...
  ukUA = {
    ...
    APPLICANT_TOOLTIP_SUPPLEMENT = "доповнення",
    ...
    APPLICANT_DUMP_MEMBER = "  учасник %s | ім'я=%s роль=%s specID=%s ilvl=%s pvp=%s score=%s рів=%s leaver=%s listingRun=%s контекст=%s джерело=%s",
